...
/**
 *  通过关键词搜索 获取店铺信息（包括未分类的和分类的，没点进店铺）
 *  产生 ElemeUrlTaskJob
 */
public class MobileEleSearchKeywordThread extends Thread {

    //private Dao tDao = SpringContextHolder.getBean("tDao");
    private Dao tDao = MySqlDataSource.getMysqlDataSource(Constant.ZNode).getMysqlDao();
    private EleMeSearchUrlJob task;
    private static Logger logger = Logger.getLogger(MobileEleSearchKeywordThread.class);
    private boolean combine = MyConstant.MOBILE_ELEME_CONFIG.COMBINE;

    public MobileEleSearchKeywordThread(EleMeSearchUrlJob task) {
        this.task = task;
    }
    ...
    public void run() {
        ...
        try {
            ...
            tDao.update(EleMeSearchUrlJob.class, Chain.make("status", 1), Cnd.where("id", "=", task.getId()));
            ...
            if (cityId == null) {
                ...
                tDao.update(EleMeSearchUrlJob.class, Chain.make("status", 4), Cnd.where("id", "=", task.getId()));
                return;
            }
                ...
                if (array.length() > 0) {
                    ...
                    if (!judge(Double.parseDouble(cityId.getLatitude()), Double.parseDouble(cityId.getLongitude()),
                        ...
                        logger.info("==========" + "饿了么根据关键词搜索结果为：city_name与给得Adress不符，关键词为：" + task.getKeyword()
                                + "==========");
                        tDao.update(EleMeSearchUrlJob.class, Chain.make("status", 5),
                    ...
                    // 生成category数组
                    List<ElemeCategory> list = getCategories(latitude, longitude);
                    if (list.size() == 0) {// 选用固定数量的分类
                        try {
                            List<ElemeCategoryFixed> fixed = tDao.query(ElemeCategoryFixed.class, null);
                    ...
                    if (list.size() == 0) {// 避免list为空的情况，至少也要抓一遍
                        ElemeCategory kong = new ElemeCategory();
                        ...
                        for (int g = current_category; g < list.size(); g++) {
                            while (true) {
                                ...
                                if (StringUtils.isEmpty(task.getShopKeyword())) {
                                	//https://mainsite-restapi.ele.me/shopping/restaurants?latitude=45.3663&longitude=126.31198&offset=1&limit=20&extras%5B%5D=activities&restaurant_category_ids%5B%5D=207
                                    ...
                                    try {
                                        ...
                                        if (shopList.length() == 0) {
                                            ...
                                            offset = 0;// 重置
                                            k = 0;
                                            tDao.update(EleMeSearchUrlJob.class,
...
//                                            }
                                        ...
                                        offset = 20 * k;
                                        //if (k % 5 == 0) {
                                        tDao.update(EleMeSearchUrlJob.class, Chain.make("offset", offset),
                                                Cnd.where("id", "=", task.getId()));
                                        //}
                                    } catch (Exception e) {
                                        logger.error("");
...
//                                            }
                                        ...
                                        logger.info("下载饿了么第" + k + "页完成，该页共" + shop_number + "家店铺，耗时:"
                                                + (System.currentTimeMillis() - time));
                                        tDao.update(EleMeSearchUrlJob.class, Chain.make("offset", shop_number),
                                                Cnd.where("id", "=", task.getId()));
                                        total_shop += shop_number;
                                        // 有店铺关键词的话只返回一次，不翻页
                                        break;
                        ...
                        while (true) {
                            ...
                            long time = System.currentTimeMillis();
                            if (StringUtils.isEmpty(task.getShopKeyword())) {
                            	//https://mainsite-restapi.ele.me/shopping/v2/restaurant/category?latitude=45.3663&longitude=126.31198
...
//                                        }
                                ...
                                try {
                                    ...
                                    // (System.currentTimeMillis() - time));
                                    logger.info(result);
                                    if (result.contains("SERVER_UNKNOWN_ERROR")){
                                        break;
                                    }
                                    JSONArray shopList = new JSONArray(result);
                                    shop_number += shopList.length();
...
//                                        }
                    ...
                    tDao.update(EleMeSearchUrlJob.class, Chain.make("status", 2), Cnd.where("id", "=", task.getId()));
                } else {
                    tDao.update(EleMeSearchUrlJob.class, Chain.make("status", 31), Cnd.where("id", "=", task.getId()));
                }
            } catch (Exception e2) {
                logger.error("");
                tDao.update(EleMeSearchUrlJob.class, Chain.make("status", 3), Cnd.where("id", "=", task.getId()));
            }
                ...
                for (int i = 1; i < array.length(); i++) {
                    if (array.getJSONObject(i).has("sub_categories")) {
                        ...
                        for (int j = 1; j < sub_array.length(); j++) {
                            ...
                            e.setCategory(array.getJSONObject(i).getString("name")); //简食便当(第二层分类)
                            e.setSub_category(sub_array.getJSONObject(j).getString("name"));//美食(第一层分类)
                            e.setCategory_id(sub_array.getJSONObject(j).getString("id")); //-100
                            cates.add(e);
                        }
                    }
                }
