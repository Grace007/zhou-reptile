 ...
 * 获取评论
 */
public class MobileElemeCommentThread extends Thread {
    //private Dao tDao = SpringContextHolder.getBean("tDao");
    private Dao tDao = MySqlDataSource.getMysqlDataSource(Constant.ZNode).getMysqlDao();
    private static Logger logger = Logger.getLogger(MobileElemeCommentThread.class);
    private ElemeCommentTaskJob task;

    public MobileElemeCommentThread(ElemeCommentTaskJob task) {
        this.task = task;
    }
    ...
    public void run() {
        ...
        logger.info("开始下载饿了么评论...");
        TaskToBi tobi = BIDataSource.getBiDataSource("eleme", task.getProject_code(), task.getDown_type());
        Dao biDao = tobi.getBiDao();
        tDao.update(ElemeCommentTaskJob.class, Chain.make("status", 1), Cnd.where("id", "=", task.getId()));
        String shop_id = task.getShop_id();
        // https://www.ele.me/restapi/ugc/v1/restaurant/699381/ratings?limit=10&offset=0
        //https://mainsite-restapi.ele.me/ugc/v1/restaurant/699381/ratings?has_content=true&offset=1&limit=10
...
//                    logger.info("==========================手机端饿了么第" + i + "页评论解析完成，耗时：" + (System.currentTimeMillis() - comment_time)
//                            + "============================");
                    if (i < page_to) {
                        tDao.update(ElemeCommentTaskJob.class, Chain.make("page_from", (i + 1)), Cnd.where("id", "=", task.getId()));
                    }
                } catch (Exception e) {
                    logger.info("", e);
                    tDao.update(ElemeCommentTaskJob.class, Chain.make("status", 3), Cnd.where("id", "=", task.getId()));
                    return;
                }
            }
            logger.info("==========================手机端饿了么评论解析完成，总耗时：" + (System.currentTimeMillis() - time)
                    + "============================");
            tDao.update(ElemeCommentTaskJob.class, Chain.make("status", 2), Cnd.where("id", "=", task.getId()));
        }
    }
