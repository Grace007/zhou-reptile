...
public class MobileBaiduWaimaiCommentThread extends Thread {
    //private Dao tDao = SpringContextHolder.getBean("tDao");
    private Dao tDao = MySqlDataSource.getMysqlDataSource(Constant.ZNode).getMysqlDao();
    private static Logger logger = Logger.getLogger(MobileBaiduWaimaiCommentThread.class);
    private BaiduWaimaiCommentTaskJob task;

    public MobileBaiduWaimaiCommentThread(BaiduWaimaiCommentTaskJob task) {
        this.task = task;
    }
    ...
    public void run() {
        ...
        tDao.update(BaiduWaimaiCommentTaskJob.class, Chain.make("status", 1), Cnd.where("id", "=", task.getId()));
        ...
        if (StringUtils.isNotEmpty(shop_id)) {
            ...
            for (int i = page_from; i <= page_to; i++) {
                ...
                try {
                    ...
                    //logger.info("百度评论第" + i + "页解析耗时:" + (System.currentTimeMillis() - time));
...
//                    logger.info("==========================第" + i + "页评论URL解析完成，耗时：" + (System.currentTimeMillis() - comment_time)
//                            + "============================");
                    if (i < page_to) {
                        tDao.update(BaiduWaimaiCommentTaskJob.class, Chain.make("page_from", (i + 1)),
                                Cnd.where("id", "=", task.getId()));
                    }
                } catch (Exception e) {
                    logger.info("", e);
                    tDao.update(BaiduWaimaiCommentTaskJob.class, Chain.make("status", 3),
                            Cnd.where("id", "=", task.getId()));
                    return;
                }
            }
            logger.info("==========================评论URL解析完成，总耗时：" + (System.currentTimeMillis() - time)
                    + "============================");
            tDao.update(BaiduWaimaiCommentTaskJob.class, Chain.make("status", 2),
                    Cnd.where("id", "=", task.getId()));
        } else {
            tDao.update(BaiduWaimaiCommentTaskJob.class, Chain.make("status", -1),
                    Cnd.where("id", "=", task.getId()));
        }
    }
