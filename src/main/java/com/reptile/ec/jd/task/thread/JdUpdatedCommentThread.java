...
public class JdUpdatedCommentThread extends Thread {

	private static Logger logger = Logger.getLogger(JdUpdatedCommentThread.class);
	private static NutDao tDao = SpringContextHolder.getBean("tDao");
	private JdProductJob task;

	public JdUpdatedCommentThread(JdProductJob task) {
		this.task = task;
	}
	...
	public void run() {
		try {
			...
			Dao biDao = BIDataSource.getBiDataSource(task.getWebsite(), task.getProject_code(), "get_product_info")
					.getBiDao();
			tDao.update(JdProductJob.class, Chain.make("comment_status", 5), Cnd.where("id", "=", task.getId()));
			...
			Long updatedComments = task.getUpdated_comments();
			int maxPage = 0;
			if (updatedComments % 30 == 0) {
				maxPage = (int) (Long.valueOf(updatedComments) / 30);
			} else {
				maxPage = (int) (Long.valueOf(updatedComments) / 30 + 1);
			}
			...
			for (int i = curPage; i <= maxPage; i++) {
				...
				try {
					...
					if ((!html.contains("全部评价")) && (!html.contains("全部商品评价"))) {
						// 错误页面，跳出
						tDao.update(JdProductJob.class, Chain.make("comment_status", 4).add("commentsPage", i),
					...
					logger.info("###########已經完成," + task.getPid() + "第" + i + "頁下載#############");
				} catch (Exception e) {
