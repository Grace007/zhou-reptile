...
import com.reptile.ec.amazon.task.thread.AmazonProductThread;
...
@Service
public class AmazonProductTask extends BaseTask<AmazonProductJob> {
	private Dao tDao = SpringContextHolder.getBean("tDao");
	private static Logger logger = Logger.getLogger(AmazonProductTask.class);
	...
	public void doStart() {
		...
		if(MyConstant.AMAZON_CONFIG.INFO){
			...
			tDao.update(AmazonProductJob.class,Chain.make("status", 0),Cnd.where("status","=",1).and("node", "=", MyConstant.Node));
			try {
				if (QuartzManager.getTrigger("AmazonProductThread") == null) {
					QuartzManager.addJob("AmazonProductThread", this, "0/30 * * * * ?");
				} else {
					logger.info("已经存在:AmazonProductThread");
				}
	...
	public Thread getThread(AmazonProductJob task) {
		// TODO Auto-generated method stub
		return new AmazonProductThread(task);
	}
