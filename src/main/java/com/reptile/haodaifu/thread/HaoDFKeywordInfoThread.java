...
import com.reptile.haodaifu.model.HaoDFKeywordInfo;
...
public class HaoDFKeywordInfoThread extends Thread {

	private Dao mysqlDao = MySqlDataSource.getMysqlDao(MyConstant.ZNode);
	private HaoDFKeywordSimpleJob task;
	private Logger logger =Logger.getLogger(HaoDFKeywordInfoThread.class);
	TaskToBi tobi = BIDataSource.getBiDataSource("doctor", "DOCTOR", "get_article_info");
	Dao biDao = tobi.getBiDao();
	public  HaoDFKeywordInfoThread(Record task) {
		this.task = task.toPojo(HaoDFKeywordSimpleJob.class);
	}
	
	@Override
	public void run() {
		try {
			mysqlDao.update(HaoDFKeywordSimpleJob.class, Chain.make("status", 1), Cnd.where("id", "=", task.getId()));
			Elements wendaElements=null;
			Element wendaElement =null;
			String spokesman="",wenda_title="",wenda_time="",patient_status="",wenda_content="",wenda_type="",did="";
			String baseUrl =task.getUrl();
			String html="";
			int ranking=1,pagenum=1,pagetotal=1;
			
			HaoDFKeywordInfo info = new HaoDFKeywordInfo();
			...
			info.setWenda_url(task.getUrl());
			info.setWenda_id(task.getIid());
			...
			for (int i = 0; i < wendaElements.size(); i++) {
				...
				info.setRanking(ranking);
				info.setMd5(MD5Util.getMD5(task.getRequest_id()+task.getTask_id()+task.getUrl()+wenda_content+wenda_type));
				try {
					biDao.insert(info);
				} catch (Exception e) {
				}
			...
			logger.info("HaoDFKeywordinfo任务完成,pagenum="+pagenum+"   pagetotal="+pagetotal);

			mysqlDao.update(HaoDFKeywordSimpleJob.class, Chain.make("status", 2), 
					Cnd.where("id", "=", task.getId()));
		} catch (Exception e) {
			mysqlDao.update(HaoDFKeywordSimpleJob.class, Chain.make("status", 3), 
					Cnd.where("id", "=", task.getId()));
			logger.error("HaoDFKeywordinfo出错",e);
		}
	}
