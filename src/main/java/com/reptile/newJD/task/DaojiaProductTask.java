...
import com.reptile.newJD.thread.DaojiaProductThread;
...
@Service
public class DaojiaProductTask extends BaseTask<JDDaoJiaProductUrlJob> {

	private static Logger logger = Logger.getLogger(DaojiaProductTask.class);
	...
	public ExecutorService getPool() throws Exception {
		return ThreadPools.getThreadPool(this,
				MyConstant.JDDAOJIA_CONFIG.JDDAOJIA_PRODUCT_THREAD);
	}

	@Override
	public List<JDDaoJiaProductUrlJob> getTaskList() throws Exception {
		long t = System.currentTimeMillis();
		List<JDDaoJiaProductUrlJob> list = tDao
				.query(JDDaoJiaProductUrlJob.class,
						Cnd.where("status", "=", 0)
								.and("node", "=", MyConstant.Node)
								.limit(1,
										MyConstant.JDDAOJIA_CONFIG.JDDAOJIA_PRODUCT_LIMIT));
		logger.info("======JDDaojiaProductTask从任务库取出" + list.size() + "个任务，耗时："
	...
	public void doStart() {
		if (MyConstant.JDDAOJIA_CONFIG.JDDAOJIA_PRODUCT) {
			...
			tDao.update(
					JDDaoJiaProductUrlJob.class,
					Chain.make("status", 0),
					Cnd.where("status", "=", 1).and("node", "=",
							MyConstant.Node));
			try {
				if (QuartzManager.getTrigger("JDDaoJiaProductThread") == null) {
					QuartzManager.addJob("JDDaoJiaProductThread", this,
							MyConstant.JDDAOJIA_CONFIG.JDDAOJIA_PRODUCT_TIME);
				} else {
	...
	public Thread getThread(JDDaoJiaProductUrlJob task) {
		return new DaojiaProductThread(task);
	}
